...
import { Plus, Edit2, Trash2, Search, ChevronDown, ChevronUp, Loader2 } from 'lucide-react'
...
export default function PendenciasPage() {
  ...
  useEffect(() => {
    fetchPendencias()
      .then(setItems)
      .catch(() => toast('Erro ao carregar pendências.', 'error'))
  ...
  const save = async () => {
    ...
    try {
      ...
      if (editItem) {
        const updated = await updatePendencia(editItem.id, { ...payload, updated_by: user?.name || 'Admin' })
        setItems(prev => prev.map(p => p.id === editItem.id ? updated : p))
        toast('Pendência atualizada!')
      } else {
        const created = await insertPendencia(payload)
  ...
  const del = async (p: Pendencia) => {
    try {
      await deletePendencia(p.id)
      setItems(prev => prev.filter(x => x.id !== p.id))
      toast(`"${p.title}" excluída.`, 'error')
    } catch {
  ...
  return (
    <div>
      ...
      {tab === 'pend' && (
        <div>
          ...
          <div className="card">
            <div className="tw">
              {loading ? (
                <div style={{ display: 'flex', justifyContent: 'center', padding: 40 }}><Loader2 size={22} className="spin" /></div>
              ) : (
                <table>
                  ...
                  <tbody>
                    ...
                    {!loading && !filtered.length && <tr><td colSpan={8} style={{ textAlign: 'center', padding: 28, color: 'var(--muted)' }}>Nenhuma pendência encontrada</td></tr>}
                  </tbody>
                </table>
              )}
            </div>
      ...
      <Modal open={showForm} onClose={() => setShowForm(false)} title={editItem ? 'Editar Pendência' : 'Nova Pendência'}
        footer={<><button className="btn bo" onClick={() => setShowForm(false)}>Cancelar</button><button className="btn bp" onClick={save} disabled={saving}>{saving ? <Loader2 size={12} className="spin" /> : null}Salvar</button></>}>
        <div className="g2">
